    ...
    % 
    % The overall governing equation for the measured current is:
    % 
    %%
    ...
    % where
    %
    % $i_{monolayer} = i_{0}e^{-Kt}$ - the current arising from the formation
    % of the initial monolayer of the oxide following the AFM tip passage
    %
    % 
    % $$i_{growth} = i_{0}e^{\eta g^{+}}/(1 +
    % k_{film}E_{film}g^{+}i_{0}e^{\eta g^{+}}\delta t)$$    
    % - the current
    % arising from the high field film growth following the initial
    % monolayer formation
    %
    % $i_{passive} = i_{0,passive}$ - the long-term background passive
    % current once the oxide film has stabilized
    %
    % Revision Record
    %
